...
var serverApi = {
  ...
  getWeather: function getWeather(place) {
    ...
    var YQL = "select * from weather.forecast where woeid in (select woeid from geo.places(1) where text=\"" + place + "\")";
...
var pubsub = new (__webpack_require__(0))();
var serverApi = __webpack_require__(2);
var storage = __webpack_require__(4);
var TEXT_ERROR_MESSAGE = 'Oops! \n Something went wrong. \n Please try again';
...
var store = {
  ...
  setMemoryCities: function setMemoryCities(data) {
    if (!data.count) return;
    var query = data._links.self.href.match(/=(.*)&/)[1];
    var list = data._embedded['city:search-results'];
  ...
  init: function init() {
    ...
    pubsub.subscribe('new-substring-on-search-cityes', function (data) {
      ...
      if (_this.memoryCities[substring]) {
      ...
      serverApi.getCitiesBySubstring(substring).then(function (response) {

        _this.setMemoryCities(response);

        _this.listSities = response._embedded['city:search-results'];
        if (!_this.listSities.length) {
    ...
    pubsub.subscribe('selected-city', function (data) {
      _this.memoryCities = {};
      var link = _this.listSities[+data.index]['_links']['city:item']['href'];
      // let QQQ = this.listSities[+data.index]['matching_full_name']
      console.log(_this.listSities[+data.index]);
      // debugger
      _this.listSities = [];
      pubsub.publish('hide-block-search');
      pubsub.publish('create-list-cityes', _this.listSities);
      ...
      serverApi.getSity(link).then(function (response) {
        var code = response._links['city:country']['href'];
        code = code.match(/[A-Z]{2}/)[0];
        // debugger
        console.log(response);
        console.log(response.name + ', ' + code);

        serverApi.getWeather(response.name + ', ' + code).then(function (response) {
          var data = response.query.results.channel;
          data.id = _this.weatherCardId++;
    ...
    pubsub.subscribe('update-all-weather-card', function () {
      storage.getStorage().then(function (response) {
        ...
        response.forEach(function (item) {
          ...
          var id = item.id;
          serverApi.getWeather(place).then(function (response) {
            var data = response.query.results.channel;
            data.id = id;
            _this.date = new Date();
...
var localforage = __webpack_require__(9);
...
var ButtonImg = __webpack_require__(1);
var pubsub = new (__webpack_require__(0))();
...
var serverApi = __webpack_require__(2);
var storage = __webpack_require__(4);
var WeathersList = __webpack_require__(11);
var offline = document.querySelector('.offline');
var Header = __webpack_require__(15);
var BlockSearch = __webpack_require__(16);
var InputSearch = __webpack_require__(6);
var ListSities = __webpack_require__(17);
var Message = __webpack_require__(19);
var NotFound = __webpack_require__(20);
...
var pubsub = new (__webpack_require__(0))();
var getParentNode = __webpack_require__(12);
...
function templateCard(data) {
  ...
  var forecast = item.forecast.slice(1, 7);
...
var pubsub = new (__webpack_require__(0))();
var SelectItemList = __webpack_require__(18);
